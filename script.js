...
(function () {
  ...
  function renderGifts() {
    ...
    WC_GIFTS.forEach(function (g, i) {
      ...
      html +=
        ...
        '<button type="button" class="wc-btn wc-btn-pix" data-gift-index="' + i + '">Pix</button>' +
    ...
    // clique no botão Pix abre o modal com QR Code + código copia-e-cola
    grid.querySelectorAll(".wc-btn-pix").forEach(function (btn) {
      btn.addEventListener("click", function () {
        var idx = Number(btn.getAttribute("data-gift-index"));
        openPixModal(WC_GIFTS[idx]);
      });
    });
  }
  ...
  function openPixModal(gift) {
    ...
    if (!modal) return;
    document.getElementById("pix-title").textContent = gift.titulo;
    document.getElementById("pix-price").textContent = "R$ " + formatBRL(gift.preco);
    document.getElementById("pix-qr").src = gift.pixQr;
    document.getElementById("pix-code").value = gift.pixCode;
  ...
  // ---------- Anima a entrada/saída dos elementos .wc-reveal ao rolar a tela ----------
      ...
      function (entries) {
        entries.forEach(function (entry) {
          if (entry.isIntersecting) {
            ...
            // saiu da tela: some de novo, pra reaparecer quando voltar a rolar até ele
